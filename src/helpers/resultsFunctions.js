...
const renderBladesWithWidthsNoType = params => {
  const filteredBlades = _.filter(data, {cutting: [{sort: params.sort, type: null}]})
  const filteredBladesWithWidths = filteredBlades.filter(item => item.widths.indexOf(params.width) > -1)
  return filteredBladesWithWidths.map(item => {
    return (
        <div key={item.id}>
          <h1>Linia {item.line}:</h1>
...
export { renderBladesWithWidths, renderBladesWithWidthsNoType, renderBlades }